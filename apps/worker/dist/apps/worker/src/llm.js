...
import { confidenceLevels, findingCategories, severityLevels, } from '@limen/shared/schemas/finding';
...
import { rewriteFieldTypes, rewriteSuggestionSchema, } from '@limen/shared/schemas/rewrite-suggestion';
import { z } from 'zod';
...
export async function generateLaunchReportWithLlm({ runId, url, audience, trafficChannel, desiredAction, offer, objections, competitors = [], brandVoice, analysisInput, }) {
    ...
    for (const provider of providers) {
        try {
            ...
            if (!report) {
                ...
                throw new Error(`Provider ${provider} is configured but produced no report. This is a dispatch gap, not an API failure.`);
            }
...
async function generateWithAnthropic({ apiKey, payload, }) {
    const response = await fetch('https://api.anthropic.com/v1/messages', {
        ...
        body: JSON.stringify({
            ...
            max_tokens: 8192,
            temperature: 0.3,
            system: 'You are Limen, a launch-readiness analyst. Return only valid JSON matching the required shape, with no markdown fences.',
            messages: [
                {
                    role: 'user',
...
async function generateWithOpenAiCompat({ apiKey, baseUrl, model, payload, }) {
    const response = await fetch(`${baseUrl}/chat/completions`, {
        ...
        body: JSON.stringify({
            ...
            messages: [
                {
                    role: 'system',
                    content: 'You are Limen, a launch-readiness analyst. Return only valid JSON matching the required shape.',
                },
                {
                    role: 'user',
                    content: buildPrompt(payload),
...
function parseLaunchReport(text) {
    let raw;
    try {
...
function normalizeLaunchReport(report) {
    return {
        ...report,
        ...
        rewrites: report.rewrites
            .filter((rewrite) => rewriteFieldTypes.includes(rewrite.fieldType))
            .slice(0, 4),
    };
}
